...
class FnacProvider:
    ...
    def _setup_chrome(self, use_profile: bool = True) -> webdriver.Chrome:
        ...
        logger.info(
            "FNAC: lancement Chrome (profil: %s)", profile_path or "non (temporaire)"
        )
        ...
        if (
            ...
            if candidate.exists():
                logger.info(
                    "FNAC: correction chemin ChromeDriver (%s -> %s)",
                    driver_path,
                    candidate,
                )
    ...
    def _is_logged_in(self) -> bool:
        ...
        if any(p in url for p in authenticated_paths) and "connexion" not in url:
            logger.debug(
                "FNAC _is_logged_in: URL espace client -> connecte (%s)", url[:80]
            )
            return True
        try:
    ...
    async def navigate_to_invoices(self) -> bool:
        ...
        if self._is_orders_page():
            logger.info(
                "FNAC: déjà sur la page commandes (%s)", self.driver.current_url[:80]
            )
            return True
        ...
        for path in FNAC_ORDERS_PATHS:
            ...
            if self._is_orders_page():
                logger.info(
                    "FNAC: page commandes trouvée (%s)", self.driver.current_url[:80]
                )
                return True
        ...
        try:
            for a in self.driver.find_elements(By.TAG_NAME, "a"):
                ...
                if "mes commandes" in text or (
                    "commande" in href and "fnac.com" in href
                ):
                    a.click()
                    time.sleep(3)
                    if self._is_orders_page():
                        logger.info(
                            "FNAC: page commandes atteinte via lien (%s)",
                            self.driver.current_url[:80],
                        )
                        return True
                    break
            ...
            pass
        logger.warning(
            "FNAC: impossible d'atteindre la page des commandes. URL: %s",
            (self.driver.current_url or "")[:80],
        )
        return False
        ...
        for el in self.driver.find_elements(
            ...
            try:
                ...
                if tag == "a":
                    ...
                    logger.info(
                        "FNAC: bouton facture trouvé ('%s'), clic...",
                        (el.text or "")[:40],
                    )
                    el.click()
                    time.sleep(3)
                    ...
                    for a in self.driver.find_elements(
                        ...
                        if href:
                            logger.info(
                                "FNAC: lien PDF après clic bouton: %s", href[:80]
                            )
                            return href
            except Exception:
                continue
            ...
            try:
                ...
                if (
                    ...
                    or (
                        ...
                        and any(
                            x in href_lower
                            for x in ["order", "commande", "invoice", "facture"]
                        )
                    )
                ):
                    logger.info(
                        "FNAC: lien facture trouvé (scan): %s | texte: %s",
                        href[:80],
                        text_lower[:40],
                    )
                    return href
            ...
            for a in self.driver.find_elements(By.TAG_NAME, "a"):
                try:
                    ...
                    if any(
                        ...
                        continue
                    if re.search(
                        r"/account/order/(resale|return|wishlist|sav)", href_lower
                    ):
                        continue
                    if any(
                        x in href_lower
            ...
            for a in self.driver.find_elements(By.TAG_NAME, "a"):
                try:
                    ...
                    if (
                        ...
                        or "suivant" in aria
                        or text in (">", "›", "»", "next")
                        or "page suivante" in text
                        or "page suivante" in aria
                    ):
                        full = (
                            urljoin(base, href) if not href.startswith("http") else href
                        )
                        logger.info(
                            "FNAC: page suivante trouvée via lien: %s", full[:80]
                        )
                        return full
                except Exception:
                    continue
    ...
    def _collect_all_orders(self, max_pages: int = 20) -> List[OrderInfo]:
        ...
        while page_num <= max_pages:
            ...
            if not new:
                logger.info(
                    "FNAC: pagination — page %d : aucune nouvelle commande, arrêt",
                    page_num,
                )
                break
            for o in new:
            ...
            logger.info(
                "FNAC: pagination — page %d : %d commande(s) ajoutée(s), total: %d",
                page_num,
                len(new),
                len(all_orders),
            )
            ...
            try:
                ...
                logger.warning(
                    "FNAC: pagination — erreur chargement page %d: %s", page_num + 1, e
                )
                break
            page_num += 1
        logger.info(
            "FNAC: %d commande(s) collectée(s) au total (%d page(s))",
            len(all_orders),
            page_num,
        )
        return all_orders
    ...
    def _rename_browser_download(
        ...
        if path.name != new_name:
            try:
                ...
                logger.warning(
                    "FNAC: impossible de renommer %s -> %s : %s", path.name, new_name, e
                )
                return path.name
        return new_name
    ...
    async def download_invoices(
        ...
        for i, order in enumerate(filtered):
            ...
            await self._notify_progress(
                on_progress, count, total, f"Analyse commande {i + 1}/{total}…"
            )

            if not force_redownload and self.registry.is_downloaded(
                PROVIDER_FNAC, order.order_id
            ):
            ...
            if ".pdf" in url_lower:
                ...
                for suffix in ["/invoice", "/facture", "/invoice/download"]:
                    ...
                    direct = self._download_pdf(
                        candidate, order.order_id, order.invoice_date
                    )
                    if direct:
                        self.registry.add(
                            PROVIDER_FNAC,
                        ...
                        logger.info(
                            "FNAC: facture via URL directe %s ->%s", suffix, direct
                        )
                ...
                try:
                    ...
                    if not invoice_pdf_url:
                        ...
                        if downloaded_path:
                            fname = self._rename_browser_download(
            ...
            filename = self._download_pdf(
                invoice_pdf_url, order.order_id, order.invoice_date
            )
            if not filename:
                # Fallback : laisser le navigateur télécharger (gère les tokens JS/auth React)
                logger.info(
                    "FNAC: _download_pdf échoué, fallback navigateur pour %s",
                    invoice_pdf_url[:80],
                )
                ...
                try:
                    ...
                    downloaded_path = self._wait_for_browser_download(
                        existing_before_browser, max_wait=30
                    )
                    if downloaded_path:
